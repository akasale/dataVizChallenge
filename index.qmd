...
**Answer:** 
At high altitudes, the thinner air causes baseballs to travel farther than they would at lower elevations. To offset this effect, Coors Field was designed with outfield fences set unusually far from home plate, making it the largest outfield in Major League Baseball. Despite the extended fences, for many years the park still yielded the most home runs in the league—and because of its vast outfield, it also produced the highest number of doubles and triples.
:::
...
**Answer**
```{python}
totalGames = len(df2010) + len(df2021)
print(f"\n{len(df2010)} games in 2010 and {len(df2021)} games in 2021 adding to {totalGames} games in the dataset")

homeRun2010 = df2010.groupby('home').size().sort_values(ascending=False)
print(homeRun2010.head(3))

homeRun2021 = df2021.groupby('home').size().sort_values(ascending=False)
print(homeRun2021.head(3))
```
:::
By summarizing results into stadium averages, we can clearly observe the average number of runs and home runs scored at each stadium in a given year. This approach highlights differences in performance across venues and helps identify which stadiums are most conducive to high scoring.

**Benefits of aggregation:**

- Reduces random noise
- Reveals underlying patterns
- Produces cleaner data and clearer visualizations
...
**Answer**
- A horizontal bar chart clearly shows how stadium averages differ, while a scatter plot would obscure categorical comparisons.
- A vertical bar chart will be useful for chronological or short-label data (like seasons or months)
...
- Color highlighting adds emphasis and supports interpretability — it communicates importance or exception within the broader dataset. The Colorado (COL) stadium — within a larger comparison of average runs per stadium.
- Position (x/y) is best for representing data magnitude — exact comparisons. Color is best for categorical distinction or highlighting — drawing attention to a subset or pattern.
:::
...
**Answer**
- The visualization follows storytelling design principles — it’s simple, focused on one main insight, and visually polished enough for decision-makers to interpret quickly.
- The annotation is helpful because it draws attention to the data point that supports the  headline.

However, the current placement at (colorado_runs + 0.5, colorado_index) can sometimes overlap with the rightmost edge or extend too far, depending on scaling.

```{python}
# Improved annotation placement: offset vertically instead of horizontally
ax.annotate(f"COL: {colorado_runs:.2f} runs/game",
            xy=(colorado_runs, colorado_index),
            xytext=(max(avgDF_2010_sorted.totalRuns) * 0.75, colorado_index + 0.8),
            arrowprops=dict(arrowstyle='->', color='darkorchid', lw=2),
            fontsize=10, fontweight='bold', color='darkorchid',
            bbox=dict(boxstyle="round,pad=0.3", fc="white", ec="darkorchid", alpha=0.6))

league_avg = avgDF_2010_sorted.totalRuns.mean()
ax.axvline(league_avg, color='grey', linestyle='--', lw=1)
ax.text(league_avg + 0.1, 0.5, f"League Avg: {league_avg:.2f}", 
...
print(f"\nSummary Statistics for 2010:")
print(f"Colorado (COL) average runs per game: {colorado_runs:.2f}")
print(f"League average runs per game: {league_avg:.2f}")
print(f"Colorado is {((colorado_runs / league_avg) - 1) * 100:.1f}% above league average")
```
:::
The fixed annotation 

- Moves the annotation upward and leftward, keeping it inside the chart area.
- Uses a white semi-transparent background box (bbox) to ensure text remains legible.
- Keeps the arrow clean and visible while avoiding crowding.
...
AI (I used chatGPT) came up with a technical explaination (around constructs used in the Python code). It also did not come to the right conclusion to begin with. The prompt had to be rephrased to get a reasonable answer as it was not understanding the dataset and visualization properly.
:::
